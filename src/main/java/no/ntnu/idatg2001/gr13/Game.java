...
public record Game(@Getter Player player, @Getter Story story, @Getter List<Goal> goals) {
  ...
  public static Game setup() {
    ...
    startingRoom.addLink(new Link("Enter Cave"));
    startingRoom.addLink(new Link( "Exit Cave"));
    ...
    Link magicSpell = new Link("Cast Magic Spell");
    ...
    Passage spellRoom = new Passage("Go Right", "<: You've entered a seemingly empty room," +
        " it's very dark! :>");
    caveEntrance.addLink(new Link("Go Left"));
    caveEntrance.addLink(new Link("Go Right"));
