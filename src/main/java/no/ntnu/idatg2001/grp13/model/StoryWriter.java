...
public class StoryWriter {
  ...
  public static void writeToFile(Story story, String fileName) {
    ...
    try (BufferedWriter writer = newBufferedWriter(Path.of(fileName))) {
      ...
      ErrorLogger.LOGGER.log(Level.SEVERE,
          String.format("Error writing to file: %s %s", fileName, e));
   ...
   * @throws IOException if an input/output occurs while writing to the file
   */
  private static void writePassages(Story story, Passage passage, BufferedWriter writer)
      throws IOException, IllegalArgumentException {
    if (story == null || story.getTitle().isBlank()) {
      throw new IllegalArgumentException(
          "Story must contain character and be initialized to write passages!");
    }
